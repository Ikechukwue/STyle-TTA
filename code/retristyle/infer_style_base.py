"""StyleID implementation for training-free style transfer.

Reference implementation: https://github.com/jiwoogit/StyleID
"""
...
class StyleIDMethod:
    ...
    def __init__(
        ...
        """Initialize the StyleID transfer method."""
    ...
    def get_default_config(self) -> dict:
        """Return the default inference configuration."""
        return {
    ...
    def get_native_image_size() -> int:
        """Return the default Stable Diffusion image resolution."""
        return 512
    ...
    def _ddim_inversion(
        ...
        generate the given latent. During this process, attention K/V values are
        extracted from the specified layers for later style injection.
    ...
    def _ddim_sampling_with_injection(self, noisy_latent: torch.Tensor) -> torch.Tensor:
        ...
        During sampling, style attention features are injected:
        - Query (Q): mix of content and stylized queries (controlled by gamma)
        - Keys (K): from the style image
        - Values (V): from the style image
        - Temperature (T): scales attention weights
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampling", leave=False)):
            ...
            # When index >= start_step, injection is not applied yet.
