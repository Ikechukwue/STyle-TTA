...
@description:
Dataset class for MetaShift. MetaShift leverages the Visual Genome project to
provide natural distribution shifts across different visual contexts for the same
object class (e.g., "cat(sofa)" vs "cat(bed)").

Single-domain domain generalization setup:
    - Train: Stratified training split of the source context.
...
Download & Preparation:
    MetaShift:
        ...
        2. Follow the instructions in the repository to generate subsets.
           The generate_full_MetaShift.py script creates ImageFolder-style
           directories for selected classes and contexts.
        3. Organize the generated data into ImageFolder format with context
           folders at the top level and class subfolders within.
    ...
    Concrete example (cat-vs-dog with indoor/outdoor contexts):
        <data_path>/metashift/
        ├── indoor/
        │   ├── cat/
        │   │   └── ...
    ...
    The actual context and class names depend on the MetaShift subset generated.
    The dataset auto-discovers available contexts from the directory structure.

@references:
- Weixin Liang, James Zou. "MetaShift: A Dataset of Datasets for Evaluating
    Contextual Distribution Shifts and Training Conflicts." ICLR 2022.
...
class MetaShift(VisionDataset):
    """
    Dataset class for MetaShift.
    ...
    def __init__(self, root_dir: str, split: str,
                 ...
                 **kwargs) -> None:
        """
        Initialize the MetaShift dataset.
        ...
        """
        super(MetaShift, self).__init__(root_dir, transform=transform,
    ...
    def _check_dir(path: str, name: str) -> None:
        if not os.path.isdir(path):
            raise FileNotFoundError(
                ...
                f"Please generate MetaShift data following the repository "
                f"instructions: https://github.com/Weixin-Liang/MetaShift"
            )
