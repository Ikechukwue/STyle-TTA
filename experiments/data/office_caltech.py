...
@description:
Dataset class for Office-Caltech (4 domains, 10 shared classes).

Single-domain domain generalization setup:
    - Train: Stratified training split of the source domain (default: amazon).
...
Download & Preparation:
    Download from: https://people.eecs.berkeley.edu/~jhoffman/domainadapt/
    Files needed:
        - amazon.tar.gz
        - caltech.tar.gz (Caltech-256 subset)
        - dslr.tar.gz
        - webcam.tar.gz

    Extract each into <data_path>/office_caltech/ with the following structure.
    Each domain folder should contain subfolders for each of the 10 shared classes.

    Directory structure:
        <data_path>/office_caltech/
        ├── amazon/
        │   ├── back_pack/
        │   ├── bike/
        │   ├── calculator/
...
@references:
- Paper: Boqing Gong, et al. "Geodesic Flow Kernel for Unsupervised Domain
...
class OfficeCaltech(VisionDataset):
    """
    Dataset class for Office-Caltech.
    ...
    def __init__(self, root_dir: str, split: str,
                 ...
                 **kwargs) -> None:
        """
        Initialize the Office-Caltech dataset.

        Args:
            root_dir (str): Root directory containing domain folders.
            split (str): Dataset split. One of:
                "train", "val", "test", "test_amazon", "test_caltech",
                "test_dslr", "test_webcam".
    ...
    def _check_dir(path: str, name: str) -> None:
        if not os.path.isdir(path):
            raise FileNotFoundError(
                ...
                f"Please download Office-Caltech and extract it there."
            )
