...
@description:
Dataset class for DomainNet (6 domains, 345 classes).

Single-domain domain generalization setup:
    - Train: Stratified training split of the source domain (default: real).
    - Val: Stratified validation split of the source domain.
    - Test: All non-source domains concatenated.
    - Test per domain: Individual non-source domains (test_clipart, etc.).

Download & Preparation:
    Download the cleaned version (recommended) from:
        http://ai.bu.edu/M3SDA/

    Individual domain zips:
    ...
    Extract each zip into <data_path>/domainnet/ so that each domain has
    subfolders for each of the 345 classes.
...
@references:
- Paper: Xingchao Peng, et al. "Moment Matching for Multi-Source Domain
...
class DomainNet(VisionDataset):
    ...
    def __init__(self, root_dir: str, split: str,
                 ...
                 **kwargs) -> None:
        """
        Initialize the DomainNet dataset.

        Args:
            root_dir (str): Root directory containing domain folders.
            split (str): Dataset split. One of:
                "train", "val", "test", "test_clipart", "test_infograph",
                "test_painting", "test_quickdraw", "test_real", "test_sketch".
    ...
    def _check_dir(path: str, name: str) -> None:
        if not os.path.isdir(path):
            raise FileNotFoundError(
                ...
                f"Please download DomainNet and extract it there."
            )
