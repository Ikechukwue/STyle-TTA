...
@description:
Dataset class for PACS (Photo, Art Painting, Cartoon, Sketch).

Single-domain domain generalization setup:
    - Train: Stratified training split of the source domain (default: photo).
    - Val: Stratified validation split of the source domain.
    - Test: All non-source domains concatenated.
    - Test per domain: Individual non-source domains (test_art_painting, etc.).

Download & Preparation:
    Download via DomainBed:
        python3 -m domainbed.scripts.download --data_dir=./data
    Or download from the PACS original source:
        https://drive.google.com/uc?id=1JFr8f805nMUelQWWmFnJR3y4_SYoN5Pd
    Extract into <data_path>/pacs/ with the following structure.
...
@references:
- Paper: Da Li, et al. "Deeper, Broader and Artier Domain Generalization."
    ICCV 2017.
...
class PACS(VisionDataset):
    ...
    def __init__(self, root_dir: str, split: str,
                 ...
                 **kwargs) -> None:
        """
        Initialize the PACS dataset.

        Args:
            root_dir (str): Root directory containing domain folders.
            split (str): Dataset split. One of:
                "train", "val", "test", "test_art_painting", "test_cartoon",
                "test_photo", "test_sketch".
    ...
    def _check_dir(path: str, name: str) -> None:
        if not os.path.isdir(path):
            raise FileNotFoundError(
                ...
                f"Please download PACS and extract it there."
            )
