...
HAS_MPL = True
from code.config.constants import ALL_CLASSIFIERS, TTA_STRATEGIES, ALL_SEEDS, VIT_CLASSIFIERS, VLM_CLASSIFIERS, CNN_CLASSIFIERS, FM_CLASSIFIERS
from code.config.helpers import calc_top_k
...
def generate_all_plots(args):
    ...
    for method in ['hybrid_tta', 'geometric_tta' ,'ablation/adain_tta',  'ablation/style_tta', ]:
        done.append(method)
        plot_ablation_nrefs_multi(results_dir, output_dir, done, args)
        anchors["best_n_refs"] = 16 #if method == 'ablation/style_tta' else 32
