...
def slerp(t, v0, v1):
    ...
    v0_norm = v0_copy / th.norm(v0_copy, dim=1, keepdim=True)
    v1_norm = v1_copy / th.norm(v1_copy, dim=1, keepdim=True)
    # Dot product with the normalized vectors (can be thought of as cosine of the angle)
    dot = th.sum(v0_norm * v1_norm, dim=1, keepdim=True)
...
def count_flops_attn(model, _x, y):
    ...
    # The first computes the weight matrix, the second computes the combination
...
class QKVAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        weight = th.softmax(weight.float(), dim=-1).type(weight.dtype)
        a = th.einsum(
            "bts,bcs->bct", weight, v.view(bs * self.n_heads, ch, length)
        )
        return a.reshape(bs, -1, length)
...
class UNetModel(nn.Module):
    ...
    :param conv_resample: if True, use learned convolutions for upsampling and
        downsampling.
    :param dims: determines if the signal is 1D, 2D, or 3D.
    :param num_classes: if specified, then this model will be class-conditional
    ...
    def setattr_layers(self, n_layers):
        for i in range(n_layers):
            setattr(self, f"layer_{i}", DeltaBlock(self.model_channels*8, self.model_channels*8, 0.0, dims=2, use_checkpoint=False, use_scale_shift_norm=True))


class DeltaBlock(ResBlock):
    def __init__(
        self,
        channels,
        emb_channels,
