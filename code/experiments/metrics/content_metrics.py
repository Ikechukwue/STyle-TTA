...
from code.config.paths import MODEL_DIR
...
def _get_hed_model():
    ...
    if _hed_model is None:
        from code.experiments.metrics.edge_models.hed import Network
        _hed_model = Network()
...
def _get_ldc_model():
    ...
    if _ldc_model is None:
        try:
            ...
            from code.experiments.metrics.edge_models.ldc import LDC
            
            _ldc_model = LDC()
            model_path = MODEL_DIR / "edge_detection" / "ldc_biped.pth"
...
def _get_adists_model():
    ...
    if _adists_model is None:
        try:
            ...
            from code.experiments.metrics.similarity_models.adists import ADISTS
