"""
StyleInjectionDiffusion — Customised StyleID with overloaded attention.

Core diffusion component for RetriStyle-TTA.  Based on StyleID (CVPR 2024)
with the following critical modifications:

1. **Dual-input attention**: The self-attention blocks accept *either* a full
   reference image (which is encoded on-the-fly) **or** explicitly
   pre-computed K and V tensors.  The test image always provides Q.
2. **KV-cache support**: Pre-computed K/V dicts can be passed directly,
   skipping the expensive DDIM-inversion of reference images.
3. **AdaIN latent warm-start**: Channel-wise mean/std transfer in latent
   space before denoising.

Reference: Sections 3.3, 4.2.2, 5.1 of the RetriStyle-TTA report.
"""
...
# Type aliases
# ---------------------------------------------------------------------------
# A KV-cache is a mapping:
#   layer_name -> { timestep_int: (K_tensor, V_tensor) }
KVCache = Dict[str, Dict[int, Tuple[torch.Tensor, torch.Tensor]]]

# A full feature cache additionally stores Q for content:
#   layer_name -> { timestep_int: (Q, K, V) }
    ...
    def transfer(
        ...
        * ``style_kv_cache``: Pre-computed ``{layer -> {t -> (K, V)}}``
        ...
        if style_kv_cache is not None:
            ...
            # For AdaIN we still need style latent statistics; approximate from
            # the V tensors' mean (or skip AdaIN when cache is used).
