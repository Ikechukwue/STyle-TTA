...
@description:
Dataset class for VisDA-17 (Visual Domain Adaptation Challenge 2017,
Classification track). 3 domains: train (synthetic), validation (real),
test (real).

Single-domain domain generalization setup:
    - Train: Stratified training split of the source domain (default: train).
    - Val: Stratified validation split of the source domain.
    - Test: All non-source domains concatenated.
    - Test per domain: Individual domains (test_train, test_validation, test_test).

Download & Preparation:
    Download from: http://ai.bu.edu/visda-2017/
    Direct links:
        - Train (synthetic): http://csr.bu.edu/ftp/visda17/clf/train.tar
        - Validation (real): http://csr.bu.edu/ftp/visda17/clf/validation.tar
        - Test (real): http://csr.bu.edu/ftp/visda17/clf/test.tar

    Extract into <data_path>/visda17/ with the following structure.
    ...
    Directory structure:
        ...
        │   └── truck/
        ├── validation/
        │   └── (same 12 class folders)
        └── test/
            └── (same 12 class folders)

@references:
- Paper: Xingchao Peng, et al. "VisDA: The Visual Domain Adaptation Challenge."
    arXiv 2017.
...
class VisDA17(VisionDataset):
    """
    Dataset class for VisDA-17 Classification track.

    Domains: train (synthetic), validation (real), test (real)
    Classes: 12
    """

    DOMAINS = ['train', 'validation', 'test']
    DEFAULT_TRAIN_DOMAIN = 'train'
    ...
    def _check_dir(path: str, name: str) -> None:
        if not os.path.isdir(path):
            raise FileNotFoundError(
                ...
                f"Please download VisDA-17 and extract it there."
            )
