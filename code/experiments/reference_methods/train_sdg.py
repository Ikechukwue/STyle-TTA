...
@description:
Training script for Single Domain Generalization (SDG) methods.
Supports SDG methods that improve domain generalization from a single source domain.
...
# Import SDG modules
from code.experiments.reference_methods.sdg.algorithms import ALGORITHMS, get_algorithm
from code.experiments.reference_methods.sdg.networks import create_featurizer_classifier, get_network_state_dict_for_timm
from code.experiments.reference_methods.sdg.hparams import get_hparams, parse_hparams_string
...
def save_model(accelerator: Accelerator, algorithm, final_model_path: Path, classifier_name: str, num_classes: int):
    ...
    Args:
        ...
        algorithm: SDG algorithm with network attribute
...
def train_one_epoch(
    ...
    """
    Train for one epoch using SDG algorithm.
    
    Args:
        algorithm: SDG algorithm instance
...
def train(
    ...
    **kwargs
    ...
    """
    Main training function for SDG algorithms.
    ...
    Args:
        ...
        augmentations: List of augmentation names
        sdg_algorithm: Name of SDG algorithm
    ...
    run_name = f"{dataset}-{classifier}-{sdg_algorithm}-seed{seed}"
    ...
    algorithm_hparams = get_hparams(sdg_algorithm, hparams)
    ...
    # Create SDG algorithm
    accelerator.print(f"Creating algorithm: {sdg_algorithm}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Train classification model with SDG (Single Domain Generalization) algorithms',
    ...
    parser.add_argument('--sdg_algorithm', type=str, default='JiGen',
                       choices=list(ALGORITHMS.keys()),
                       help=f'SDG algorithm. Options: {list(ALGORITHMS.keys())}')
    parser.add_argument('--hparams', type=str, default=None,
                       help='Algorithm hyperparameters as JSON string (e.g., \'{"jig_weight": 0.5}\')')
    ...
    parser.add_argument('--wandb_project', type=str, default='sdg-training', help='WandB project name')
    ...
    train(
        ...
        sdg_algorithm=args.sdg_algorithm,
        hparams=hparams,
        epochs=args.epochs,
