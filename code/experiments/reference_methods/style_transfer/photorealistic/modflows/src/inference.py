...
from code.experiments.reference_methods.style_transfer.photorealistic.modflows.src.neural_ode import NeuralODE, train_ode
from code.experiments.reference_methods.style_transfer.photorealistic.modflows.src.encoder import Encoder, enc_preprocess
...
def run_inference(encoder, device, content_im_path, style_im_path, 
    ...
    with torch.no_grad():
        ...
        # Handle input if it's a path or PIL Image
        if isinstance(content_im_path, str):
            content_im = Image.open(content_im_path).convert('RGB')
        else:
            content_im = content_im_path.convert('RGB')
            
        if isinstance(style_im_path, str):
            style_im = Image.open(style_im_path).convert('RGB')
        else:
            style_im = style_im_path.convert('RGB')
